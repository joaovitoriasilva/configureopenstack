...
CACHES = {
    'default': {
        ...
        'LOCATION': '$controllerHostname:11211',
    },
}
...
OPENSTACK_HOST = \"$controllerHostname\"
...
if [[ "$1" != "following" ]]; then
  ...
  echo "This script will install the Horizon OpenStack controller module."
...
if [[ "$1" != "following" ]]; then
  ...
  echo "11.1 - Configuring components"
fi
sleep 2
confFilePath=$(find / -name configFileController.txt)
getVariablesFromConfigFile
apt-get install openstack-dashboard -y > /dev/null
...
# Restarting services
if [[ "$1" != "following" ]]; then
  echo "2.1 - Restarting services"
else
  echo "11.1.1 - Restarting services"
fi
sleep 2
